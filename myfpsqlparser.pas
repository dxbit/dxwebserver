...
uses
  Classes, SysUtils, myfpsqlscanner, myfpsqltree;
    ...
    If B then
      ...
      if (CurrentToken=tsqlFIRST) then
        begin
          ...
          AParent.First := TSQLSelectFirstElement(CreateElement(TSQLSelectFirstElement, aParent));
          AParent.First.Value := StrToInt(CurrentTokenString);
          GetNextToken;
        end;
      if (CurrentToken=tsqlSKIP) then
        begin
          GetNextToken;
          Expect(tsqlIntegerNumber);
          AParent.Skip := TSQLSelectSkipElement(CreateElement(TSQLSelectSkipElement, aParent));
          AParent.Skip.Value := StrToInt(CurrentTokenString);
          GetNextToken;
        end;
      //
      B:=False;
...
begin
  ...
  if CurrentToken=tsqlWith then
  	Result.WithSelect := ParseWithSelectStatement(AParent);
  //
...
function TSQLParser.ParseWithSelectElement(AParent: TSQLElement
  ): TSQLWithSelectElement;
var
  Ident: TSQLIdentifierName;
begin
	Expect(tsqlIdentifier);
...
begin
  ...
  Case CurrentToken of
    ...
    tsqlFloatNumber:
      ...
      TSQLFloatLiteral(Result).Value:=StrToFloat(CurrentTokenString, FS);
      end;
    tsqlNull :
...
{$ifdef debugexpr}  Writeln('Level 4 ',TokenInfos[CurrentToken],': ',CurrentTokenString);{$endif debugexpr}
  ...
  try
    while (CurrentToken in [tsqlMul,tsqlDiv]) do
      ...
      B:=TSQLBinaryExpression(CreateElement(TSQLBinaryExpression,AParent));
      B.Left:=Result;
      // 7bit - не распознавались простейшие выражения с умножением и делением
      Result := B;
      //
      B.Right:=Right;
      Case tt of
...
begin
  ...
  GetNextToken;
  Case CurrentToken of
    tsqlSelect, tsqlWith : Result:=ParseSelectStatement(Nil,[]);
