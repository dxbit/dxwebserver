...
Type
  ...
  { TSQLSelectTableReference }

  TSQLSelectTableReference = class(TSQLTableReference)
  private
    FAliasName: TSQLIdentifierName;
    FSelect: TSQLSelectStatement;
  public
  ...
  TSQLWithSelectElement = class(TSQLElement)
  private
  ...
  public
    ...
    function GetAsSQL(Options: TSQLFormatOptions; AIndent: Integer=0): TSQLStringType;
      override;
  	property Name: TSQLIdentifierName read FName;
  ...
  TSQLWithSelectStatement = class(TSQLElement)
  private
    FRecursive: Boolean;
    FSelectList: TSQLElementList;
  public
    constructor Create(AParent: TSQLElement); override;
    destructor Destroy; override;
    function GetAsSQL(Options: TSQLFormatOptions; AIndent: Integer=0): TSQLStringType;
    ...
    property SelectList: TSQLElementList read FSelectList;
  end;

  { TSQLSelectFirstElement }

  TSQLSelectFirstElement = class(TSQLElement)
  private
    FValue: Integer;
  public
    function GetAsSQL(Options: TSQLFormatOptions; AIndent: Integer=0): TSQLStringType;
  ...
  { TSQLSelectSkipElement }

  TSQLSelectSkipElement = class(TSQLElement)
  private
    FValue: Integer;
  public
    function GetAsSQL(Options: TSQLFormatOptions; AIndent: Integer=0): TSQLStringType;
...
constructor TSQLWithSelectElement.Create(AParent: TSQLElement);
begin
  inherited Create(AParent);
  FName := TSQLIdentifierName.Create(Self);
  FFields := TSQLElementList.Create(True);
end;

destructor TSQLWithSelectElement.Destroy;
begin
  FreeAndNil(FSelect);
  FreeAndNil(FFields);
  FreeAndNil(FName);
...
function TSQLWithSelectElement.GetAsSQL(Options: TSQLFormatOptions;
  AIndent: Integer): TSQLStringType;
var
  i: Integer;
begin
  ...
  begin
    ...
    begin
      Result := Result + FFields[i].GetAsSQL(Options, AIndent);
...
constructor TSQLWithSelectStatement.Create(AParent: TSQLElement);
begin
  inherited Create(AParent);
  FSelectList := TSQLElementList.Create(True);
end;

destructor TSQLWithSelectStatement.Destroy;
begin
  FreeAndNil(FSelectList);
  inherited Destroy;
end;

function TSQLWithSelectStatement.GetAsSQL(Options: TSQLFormatOptions; AIndent: Integer
  ): TSQLStringType;
var
  i: Integer;
begin
...
destructor TSQLSelectTableReference.Destroy;
begin
  FreeAndNil(FSelect);
...
function TSQLSelectTableReference.GetAsSQL(Options: TSQLFormatOptions;
  AIndent: Integer): TSQLStringType;
begin
  if FSelect = nil then Exit('');
  Result := '(' + FSelect.GetAsSQL(Options) + ')';
  if FAliasName <> nil then Result := Result + ' ' + FAliasName.GetAsSQL(Options);
end;
...
begin
  // 7bit
  Result := '';
  if WithSelect <> nil then Result := WithSelect.GetAsSQL(Options, AIndent);
  //
  Result:=Result + SQLKeyWord('SELECT',Options);
  If Distinct then
    Result:=Result+' '+SQLKeyword('DISTINCT',Options);
  // 7bit
  If FFirst <> nil then Result:=Result+' '+ FFirst.GetAsSQL(Options);
  If FSkip <> nil then Result:=Result+' '+ FSkip.GetAsSQL(Options);
  //
  ...
  // 7bit
  if UnionAll then
  	AddExpression('UNION ALL',Union,(sfoUnionOnSeparateLine in Options),False)
	else
  //
